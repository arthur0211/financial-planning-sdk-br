...
| determinismo | `partial` | writers próprios `STORED` fecham wheel/sdist; com `setuptools==84.0.0`, o build local descartável convergiu a wheel `e0beb5a2…` e sdist `643af37b…` | igualdade Linux/Windows no mesmo freeze atual e quatro células Windows isoladas ainda não foram observadas; artifacts brutos do backend não são claim bit-reproduzível |
