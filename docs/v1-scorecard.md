...
Estado do programa: `github_source_authorized_private_staging_pending`
Autoridade: `technical_evaluation_only`
Publicação: `source_authorized_pending_remote_gates`
...
| determinismo | `partial` | writers próprios `STORED` fecham wheel/sdist; com `setuptools==84.0.0`, o build local descartável convergiu a wheel `38bf14e6…` e sdist `984c6e70…` | igualdade Linux/Windows no mesmo freeze atual e quatro células Windows isoladas ainda não foram observadas; artifacts brutos do backend não são claim bit-reproduzível |
...
| documentação | `partial` | README PT-BR/EN, ADR 0012, Apache-2.0, mantenedor, governança e checklist registram a autorização do source | canal privado e CI ainda precisam ser verificados no remoto; reviewer independente não existe |
| segurança/offline | `partial` | actions por SHA, permissões mínimas, checkout sem credencial, CodeQL/dependency review candidatos e controles históricos Linux offline/read-only | workflows ainda não executados no GitHub; secret scanning/private reporting/ruleset dependem do remoto; Windows permanece `not_observed` |
...
| preparação GitHub | onboarding, supply chain, CI e documentação pública | venv externa regular, setuptools 84/metadata v5, Apache-2.0, governança, CI expandida e autorização do owner | reviewer independente ausente | `source_authorized_private_staging_pending` | primeiro commit, remoto privado, CI real, security settings e canal privado |
