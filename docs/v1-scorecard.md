...
| segurança/offline | `partial` | Actions por SHA, permissões mínimas, checkout sem credencial, Dependabot, PVR, secret scanning e push protection ativos; CI e CodeQL públicos verdes sem alertas | dependency review do PR final ainda precisa fechar; Windows permanece `not_observed` e ruleset não substitui authority |
...
| preparação GitHub | onboarding, supply chain, CI e documentação pública | venv externa regular, setuptools 84/metadata v5, Apache-2.0, governança, CI real verde, source público, PVR e secret scanning | reviewer independente ausente | `public_source_hardening` | fechar CodeQL/dependency review e ruleset sem fingir aprovação independente |
