...
A CI candidata separa runtime SDK (Python 3.11 e 3.14), análise estática/cobertura (3.13), diagnósticos neutros no Ubuntu, integração do backend suportado e diagnósticos específicos no Windows, governança Windows, corpus matemático, CodeQL Python e dependency review. O discovery Ubuntu não executa fixtures ligadas à identidade Windows nem tenta tratar o backend Ubuntu genérico como o perfil Linux oficial `uid/gid=65532`; a suíte Windows não recebe o opt-in do live AppContainer. Todos os `uses` ficam pinados por SHA integral, `checkout` usa `persist-credentials: false`, jobs têm timeout e permissões explícitas. Execução verde continua evidência técnica self-issued, não autenticação ou authority.

A licença Apache-2.0, o mantenedor e a governança de source foram decididos no ADR 0012. Visibilidade, proteção de branch e package/release permanecem superfícies separadas; enquanto não houver gate externo implementado, nenhum artifact, package ou release pode ser promovido, mesmo que o código-fonte esteja publicamente acessível.
