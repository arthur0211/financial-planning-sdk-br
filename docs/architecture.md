...
A CI candidata separa runtime SDK (Python 3.11 e 3.14), análise estática/cobertura (3.13), diagnósticos de artifact/portabilidade, governança Windows, corpus matemático, CodeQL Python e dependency review. Todos os `uses` ficam pinados por SHA integral, `checkout` usa `persist-credentials: false`, jobs têm timeout e permissões explícitas. Esses workflows ainda precisam executar no commit remoto candidato; sua mera presença no YAML não é evidência de execução nem autenticação.
